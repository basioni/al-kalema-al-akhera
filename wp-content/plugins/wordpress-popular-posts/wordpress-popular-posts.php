...
<!-- End Wordpress Popular Posts v<?php echo $this->version; ?> -->
		...
		function get_popular_posts($instance, $return = false) {
			...
			if ( !is_array($mostpopular) || empty($mostpopular) ) { // no posts to show
				$content .= "<p>".__('لا يوجد بيانات جديدة.', 'wordpress-popular-posts')."</p>"."\n";
			} else { // list posts
				...
				foreach($mostpopular as $p) {
					...
					if ( $instance['stats_tag']['comment_count'] ) {
						...
						_n('1 تعليق', '%s تعليقات', $comments, 'wordpress-popular-posts'),
					...
					if ( $instance['stats_tag']['views'] ) {
						...
						if ($instance['order_by'] == 'avg') {
							...
							$views_text = sprintf(
							_n('1 مشاهد', '%s مشاهدين', intval($pageviews), 'wordpress-popular-posts'),
