...
class Widget_WPCC extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		<p style="margin-bottom:5px;"><label for="wpcc_currency_from">' . __('من', 'wpcc') . ':</label><br />
		<select name="wpcc_currency_from" id="wpcc_currency_from">
		' . $from_currency_options . '
		</select></p>
		
		<p style="margin-bottom:5px;><label for="wpcc_currency_to">' . __('إلي', 'wpcc') . ':</label><br />
		<select name="wpcc_currency_to" id="wpcc_currency_to">
		' . $to_currency_options . '
		</select></p>
		
		<p style="margin-bottom:10px;><label for="wpcc_currency_amount">' . __('الكمية', 'wpcc') . ':</label><br />
		<input type="text" size="4" name="wpcc_currency_amount" id="wpcc_currency_amount" />
		<input type="button" value="' . __('حول العملة', 'wpcc') . '" name="wpcc_convert" id="wpcc_convert" />&nbsp;&nbsp;<img src="' . plugins_url('wp-currency-converter/images/converting.gif') . '" alt="" id="wpcc_converting" />
		</p>
