...
function awesome_weather_logic( $atts )
{
	...
	$rtn .= "
			<div class=\"awesome-weather-header\" style='font-size:20px;'>{$header_title}</div>
	...
	if($show_stats)
	{
		...
		$rtn .= "
				...
				<div class=\"awesome-weather-todays-stats\">
					...
					<div class=\"awe_humidty\">" . __('الرطوبة:', 'awesome-weather') . " {$today->main->humidity}% </div>
					<div class=\"awe_wind\">" . __('الرياح:', 'awesome-weather') . " {$today->wind->speed}" . $speed_text . " </div>
	...
	if($days_to_show != "hide")
	{
		...
		foreach( (array) $forecast->list as $forecast )
		{
			...
			$days_of_week = array( __('الأحد' ,'awesome-weather'), __('الاثنين' ,'awesome-weather'), __('الثلاثاء' ,'awesome-weather'), __('الاربعاء' ,'awesome-weather'), __('الخميس' ,'awesome-weather'), __('الجمعة' ,'awesome-weather'), __('السبت' ,'awesome-weather') );
...
{
	if(!$msg) $msg = __('لا يوجد معلومات للطقس', 'awesome-weather');
