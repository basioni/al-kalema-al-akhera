...
function vsrp() 
{
	...
	if ( ! empty($vsrp_data) ) 
	{
		...
		foreach ( $vsrp_data as $vsrp_data ) 
		{
			...
			$vsrp_post_title = substr($vsrp_post_title, 0, 300);

			$dis_height = $dis_num_height."px";
			$vsrp_html = $vsrp_html . "<div class='vsrp_div' style='height:$dis_height;padding:2px 0px 2px 0px;direction:rtl;font-size:16px;'>"; 
			$vsrp_html = $vsrp_html . "<a href='$get_permalink'>$vsrp_post_title</a>";
			$vsrp_html = $vsrp_html . "</div>";
			$icon=plugins_url('images/new.gif',__FILE__);
			$vsrp_post_title = "<img src=$icon> ".mysql_real_escape_string(trim($vsrp_post_title));
			$get_permalink = mysql_real_escape_string($get_permalink);
			$vsrp_x = $vsrp_x . "vsrp_array[$vsrp_count] = '<div class=\'vsrp_div\' style=\'padding:10px 0px 10px 0px;direction:rtl;\'><a href=\'$get_permalink\'>$vsrp_post_title</a></div>'; ";	
			$vsrp_count++;
		...
		<div style="padding-top:8px;padding-bottom:8px;">
			<div style="text-align:right;vertical-align:middle;text-decoration: none;overflow: hidden; position: relative; margin-left: 1px; height: <?php echo $vsrp_height1; ?>;" id="vsrp_Holder">
				<?php echo $vsrp_html; ?>
			</div>
		</div>
	...
	{
		echo "<div style='padding-bottom:5px;padding-top:5px;'>لا يوجد اخبار جديدة!</div>";
	}
	wp_reset_query();
}
