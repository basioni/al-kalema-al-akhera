 ...
 * @param $posts_per_page, $offset, $order_by, $meta_key, $meta_value
 * @returns $post_ids
 */

function supernova_get_specific_post_ids($posts_per_page, $offset, $order_by, $meta_key, $meta_value){
        $post_ids = array();
       $arg = array( 'posts_per_page' => $posts_per_page, 'post_type'=>'post', 'post_status'=>'publish', 'meta_key' => $meta_key, 'meta_value'=>$meta_value,  'order_by'=>$order_by,  'offset'=>$offset);
	$the_query = new WP_Query( $arg); 
 ...
 *                       !!! UNDER CUNSTRUCTION !!!
 * 
 * NOTE :: WE WILL USE POPULAR POSTS IN THE NEXT VERSION OF THE THEME*/
...
function supernova_get_popular_posts(){        
        ...
        //echo "final_array".print_r($post_ids).'<br>';
        supernova_ajax_posts_content($post_ids, 'ajax_popular_posts_wrapper', 'ajax_popular_posts');
        echo '<button class="popular_load_more '.$total_posts.' button" title="'.$left_posts.'" '.$disabled .' >'.__('Load More', 'Supernova').'</button>';        
        die();
    }
...
function supernova_get_recommended_posts(){        
        ...
        echo '<button class="rec_load_more '.$total_pages.' button" >'.__('Load More', 'Supernova').'<img class="rec_loader" src="'.SUPERNOVA_ROOT_ADMIN.'/images/loader.gif"></button>';        
            }
        die();
    }