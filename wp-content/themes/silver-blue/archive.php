 ...
 * new template file for each specific one. For example, silver blue already
		...
		<?php if ( have_posts() ) : ?>
			<header class="archive-header">
				<h1 class="archive-title"><?php
					...
					elseif ( is_month() ) :
						printf( __( 'Monthly Archives: %s', 'silverblue' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'silverblue' ) ) . '</span>' );
					elseif ( is_year() ) :
			...
			silverblue_content_nav( 'nav-below' );
			?>
